...
@section('js')
    <script>
        ...
        function konfirmasi_upah_button(transaksi_id, participant, tanggal, konfirmasi) {
            let judul;
            let judulSukses;
            let judulGagal;
            if (!konfirmasi) {
                judul = 'Konfirmasi Pemberian Upah ke ' + participant + ' ?';
                judulSukses = 'Konfirmasi berhasil dilakukan';
                judulGagal = 'Konfirmasi gagal dilakukan';
            } else {
                judul = 'Pembatalan Pemberian Upah ke ' + participant + ' ?';
        ...
        <div class="w-full max-w-full flex-none px-3">
                ...
                <div
                    ...
                    <div class="w-1/2 max-w-full flex-none px-3 text-right">
                        @if (!$cabang->deleted_at)
                            <a href="{{ route('transaksi.create', ['isJadwal' => $isJadwal]) }}"
                            ...
                            <tbody>
                                @foreach ($transaksi as $item)
                                    <tr>
                                        ...
                                        <td class="border-b border-slate-600 bg-transparent text-left align-middle">
                                            <div>
                                                <a href="{{ route('transaksi.view', ['transaksi' => $item->id, 'isJadwal' => $isJadwal]) }}"
                                                ...
                                                @if (!$cabang->deleted_at)
                                                    ...
                                                    @else
                                                        <a href="{{ route('transaksi.edit', ['transaksi' => $item->id, 'isJadwal' => $isJadwal]) }}"
            ...
            <div class="modal" role="dialog">
                <div class="modal-box">
                    ...
                    <div>
                        <form action="{{ route('transaksi.update.status', ['isJadwal' => $isJadwal]) }}" method="POST"
                                                ...
                                                <div>
                                                    @if (!$cabang->deleted_at)
                                                        @if (!$item->konfirmasi_upah_participant)
                                                            <label for="konfirmasi_upah_button"
                                                                class="btn btn-outline btn-primary btn-sm tooltip"
                                                                data-tip="Konfirmasi Upah participant"
                                                                onclick="return konfirmasi_upah_button('{{ $item->transaksi_id }}', '{{ $item->nama_participant }}', '{{ \Carbon\Carbon::parse($item->tanggal)->format('d F Y') }}', {{ $item->konfirmasi_upah_participant }})">
                                                                <i class="ri-receipt-line text-base"></i>
                                                            </label>
                                                        @else
                                                            <label for="konfirmasi_upah_button"
                                                                class="btn btn-outline btn-error btn-sm tooltip"
                                                                data-tip="Pembatalan Upah participant"
                                                                onclick="return konfirmasi_upah_button('{{ $item->transaksi_id }}', '{{ $item->nama_participant }}', '{{ \Carbon\Carbon::parse($item->tanggal)->format('d F Y') }}', {{ $item->konfirmasi_upah_participant }})">
