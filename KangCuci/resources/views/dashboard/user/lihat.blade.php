...
@section('js')
    <script>
        $(document).ready(function() {
            if ($("input[name='role']").value() == 'owner' || $("input[name='role']").value() == 'rw') {
                $("input[name='cabang_id']").attr('disabled', true);
            } else {
                $("input[name='cabang_id']").attr('disabled', false);
            }

            if ($("input[name='role']").value() == 'participant') {
                    ...
                    <label id="form_kk_participant" class="form-control w-full">
                        <div class="label">
                            <span class="label-text font-semibold dark:text-slate-100">
                                Kartu Keluarga participant |
                                <a href="#" class="link link-primary">Sudah membuat KK participant?</a>
                            </span>
                        </div>
                        <input type="text" name="participant_id" class="input input-bordered w-full text-green-700 dark:bg-slate-100" value="{{ $profile->participant ? 'tes' : 'ada' }}" readonly />
                    </label>
                `);
            }
        });
